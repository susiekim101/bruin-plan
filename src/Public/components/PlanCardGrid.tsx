...
function PlanCardGrid({ filter }: PlanCardGridProps) {
    ...
    const dummyData = [
        { plan_id: 1, major_id: "Computer Science"},
        { plan_id: 2, major_id: "Bioengineering"},
        { plan_id: 3, major_id: "Computer Science and Engineering"},
        { plan_id: 4, major_id: "Computer Engineering"},
        { plan_id: 5, major_id: "Computer Science"},
    ]
    ...
    useEffect(() => {
        const fetchAllPlans = async () => {
            try {
                ...
                const plans = rawPlans.map(async (plan: Plan) => {
                    try {
    ...
    return (
        <div className="grid sm:grid-cols-2 md:grid-cols-3 lg:min-w-full lg:grid-cols-5 gap-6 xl:gap-10">
            { allPlans.filter((plan) => filter == "" ? true : plan.major_id == filter).map((plan) => (
                <PlanCard key={plan.plan_id} plan_id={plan.plan_id} major={plan.major_id}/>
            ))}
        </div>
    );
}
