...
export default function ShareButton({units}: ShareButtonProps) {
    ...
    return (
        <div>
            ...
            <button onClick={units >= 180 ? () => handleClick(confirmationRef) : () => handleOpenClick(dialogRef)} className="w-fit text-white py-1 px-2 text-sm border rounded-md border-blue-800 bg-blue-800 cursor-pointer transition duration-300 hover:scale-110">Share</button>
        </div>
    )
}