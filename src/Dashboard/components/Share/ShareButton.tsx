...
import { handleShareClick, handleOpenDialog, handleCloseDialog, handleCloseConfirmation, handleUnshare } from "./ShareButtonHandlers"
    ...
    return (
        <div>
            ...
            <dialog ref={dialogRef} className="p-3 w-[30%] text-center bg-cyan-700 text-white rounded-lg shadow-2xl absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 backdrop:bg-gray-500 backdrop:opacity-50">
                ...
                <button onClick={() => handleCloseDialog(dialogRef)} className="bg-yellow-400 rounded-2xl px-2 py-1 cursor-pointer text-slate-700">Close</button>
            </dialog>

            <button onClick={units >= MIN_UNITS ? () => handleShareClick(confirmationRef) : () => handleOpenDialog(dialogRef)} className="w-fit text-white py-1 px-2 text-sm border rounded-md border-blue-800 bg-blue-800 cursor-pointer transition duration-300 hover:scale-110">Share</button>
        </div>
    )
}